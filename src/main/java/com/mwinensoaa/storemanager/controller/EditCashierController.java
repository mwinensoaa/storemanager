...
public class EditCashierController implements Initializable {
    ...
    @FXML private Label lbEditCashier;
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        dpDateEmployed.setEditable(false);
    ...
    public void initI18N(ResourceBundle bundle){
        ...
        lbEditCashier.setText(bundle.getString("txtEdit"));
        btnAddCashier.setText(bundle.getString("txtSave"));
        btnFileChooser.setText(bundle.getString("txtChooseImage"));
    }

    public void setupCashierInfo(Cashier cashier, ResourceBundle bundle){
        if(bundle != null){
            initI18N(bundle);
        }
        if(cashier.getCashierId() != null){
         tfCashierID.setText(cashier.getCashierId());
         tfCashierID.setDisable(true);
        }
        if(cashier.getCashierTelephone() != null){
         tfTelephone.setText(cashier.getCashierTelephone());
        }
        if(cashier.getCashierName() != null){
          tfCashierName.setText(cashier.getCashierName());
        }
        if(cashier.getCashierPassword() != null){
           tfPassword.setText(cashier.getCashierPassword());
           tfConfirmPassword.setText(cashier.getCashierPassword());
        }
        if(cashier.getCashierDob() != null){
            dpDob.setValue(GeneralUtils.formatDate(cashier.getCashierDob()));
        }

        if(cashier.getCashierEmploymentDate() != null){
            dpDateEmployed.setValue(GeneralUtils.formatDate(cashier.getCashierEmploymentDate()));

        }
        if(cashier.getCashierType() != null){
            String cashType = cashier.getCashierType().name();
            privilegesCombo.setValue(cashType);
        }
